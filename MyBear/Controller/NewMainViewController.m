...
- (void)reloadUI3{
    ...
    [dataSource addObjectsFromArray:[self read]];
...
- (void)zirewnwuClick1:(UIButton *)btn{
    nowClickLine=btn.tag-1000;
    NSDictionary *dic=[dataSource objectAtIndex:nowClickLine];
    NSArray *chilArr=[dic objectForKey:@"arr"];
    NSString *title=[dic objectForKey:@"content"];
    
    NSArray *ar=[LDefault objectForKey:@"tempArr"];
    if (ar) {
        NSMutableArray *ar1=[[NSMutableArray alloc]initWithArray:ar];
        [ar1 addObject:dic];
        [LDefault setObject:ar1 forKey:@"tempArr"];

    }else{
        [LDefault setObject:@[dic] forKey:@"tempArr"];
    }
    
    [dataSource removeAllObjects];
    [dataSource addObjectsFromArray:chilArr];
    [myTableView reloadData];
    [self setNavLeftButtonTitle:@"返回" selector:@selector(LeftButton)];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSDictionary *atempDic=dataSource[indexPath.row];
    ...
    //saveCengJiKey:@"0"
    //}
    cell.neirong.text=[dataSource[indexPath.row] objectForKey:@"content"];
...
//    }
        ...
        NSInteger tempInt=[[atempDic objectForKey:@"arr"] count];
    if (tempInt==0) {
        cell.geshu.text=@"";
